...
set_qemukjx() {
    ...
    if ! podman start registry; then
        ...
        echo "|> SCOPE: [set_qemukjx], file [./scripts/entrypoints/poc-aio.sh]; check: 01"
        echo && echo
        #return 1
        ...
        if ! [ -f "${ROOTLESS_REGISTRIES:-[EMPTY_VARIABLE]}" ] || (find "${ROOTLESS_REGISTRIES:-[EMPTY_VARIABLE]}" -type f -mtime +7200); then
            ...
            echo "|> SCOPE: [set_qemukjx], file [./scripts/entrypoints/poc-aio.sh]; check: 02"
...
EOF
    ...
    if (podman images | grep "localhost:5000/qonq_qemukjx"); then
        ...
        echo "|> SCOPE: [set_qemukjx], file [./scripts/entrypoints/poc-aio.sh]; check: 03"
        echo
        ...
        if ! (podman rmi "${BUILT_PODMAN_ALREADY:-[EMPTY_VARIABLE]}" --force); then
            ...
            echo "|> SCOPE: [set_qemukjx], file [./scripts/entrypoints/poc-aio.sh]; check: 04"
            return 1
        fi
    ...
    if ! (podman images | grep "localhost:5000/qonq_qemukjx"); then
        ...
        echo "|> SCOPE: [set_qemukjx], file [./scripts/entrypoints/poc-aio.sh]; check: 05"
        echo && echo
        # return 1
        ...
        if ! (CCR_MODE="-checker" . ./scripts/ccr.sh && docker compose -f ./compose.yml --progress=plain build --no-cache qonq_qemukjx); then
            ...
            echo "|> SCOPE: [set_qemukjx], file [./scripts/entrypoints/poc-aio.sh]; check: 06"
            echo && echo
            return 1
        fi
        ...
        if ! (podman push localhost:5000/qonq_qemukjx:latest); then
            ...
            echo "|> SCOPE: [set_qemukjx], file [./scripts/entrypoints/poc-aio.sh]; check: 07"
            return 1
        fi
    ...
    if ! (CCR_MODE="-checker" . ./scripts/ccr.sh && docker compose -f ./compose.yml create qonq_qemukjx); then
        ...
        echo "|> SCOPE: [set_qemukjx], file [./scripts/entrypoints/poc-aio.sh]; check: 08"
        return 1
    fi
    ...
    if ! podman cp qonq_qemukjx:/app/qemukjx-tarball-pkg.tar.gz "${PACKAGING_ART_DIR:-[EMPTY_VARIABLE]}"; then
        ...
        echo "|> SCOPE: [set_qemukjx], file [./scripts/entrypoints/poc-aio.sh]; check: 09"
        return 1
    fi
    ...
    if ! (podman stop registry); then
        ...
        echo "|> SCOPE: [set_qemukjx], file [./scripts/entrypoints/poc-aio.sh]; check: 10"
        return 1
    fi
...
print_usage() {
    cat <<-END >&2
USAGE: poc-aio.sh [-options]
                - qemukjx
                - tarball
                - version
                - help
eg,
MODE="qemukjx"     . ./poc-aio.sh   # create a single tarball of dependencies for qemukjx
MODE="tarball"  . ./poc-aio.sh   # create a single tarball with all
MODE="version"  . ./poc-aio.sh   # shows script version
MODE="help"     . ./poc-aio.sh   # shows this help message
