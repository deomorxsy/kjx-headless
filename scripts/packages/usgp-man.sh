...
EOF
    ...
    if (podman images | grep "localhost:5000/qonq_shadow"); then
        BUILT_SHADOW_ALREADY=$(podman images | grep "localhost:5000/qonq_shadow" | awk '{print $3}')
        export BUILT_SHADOW_ALREADY

        echo "|> WARNING: found a previously built [localhost:5000/qonq_shadow]. Attempting to remove image to [REBUILD]..."
        echo

        if ! (podman rmi "${BUILT_SHADOW_ALREADY:-[EMPTY_VARIABLE]}" --force); then
            echo "|> Error: YOU CAN (NOT) REDO the container image. Literally, it cannot be removed for some reason. No pun intended (or was it?). Exiting now... :)"
            echo
            return 1
        fi
    ...
    echo "|> WARNING: previously built [localhost:5000/qonq_shadow] removed with sucess. ...[PASSED]"
    #
    if ! (podman images | grep "localhost:5000/qonq_shadow"); then
        echo "|> Error: could not find the localhost:5000/qonq_shadow image at the OCI registry:3.0 server. Attempting to build now..."
        echo && echo
        # return 1
        # Build the shadow container with ccr.sh to use  Podman Service as the compose tool
        if ! (CCR_MODE="-checker" . ./scripts/ccr.sh && docker compose -f ./compose.yml --progress=plain build --no-cache qonq_shadow); then
            echo "|> Error: could not run the ccr.sh script for Podman Service as the compose tool. Exiting now..."
            echo && echo
            return 1

        fi
        echo "|> Successfully built the [qonq_shadow] container with ccr.sh to use Podman Service as the compose tool. Proceeding..."
        echo && echo

        # push built image into the registry:3.0 localhost:5000 server container.
        if ! (podman push localhost:5000/qonq_shadow:latest); then
            echo "|> Error: could not push the built [qonq_shadow] image into the OCI registry:3.0 localhost:5000 server container. Exiting now..."
            echo && echo
            return 1
        fi
        echo "|> Pushed built image into the OCI registry:3.0 localhost:5000 server container. Proceeding..."
    ...
    echo "|> [qonq_shadow] image found at the localhost:5000/qonq_shadow OCI registry:3.0 server. Proceeding..."

    # Create the built qonq_shadow container
    # podman run -it --name qonq_shadow -d localhost:5000/qonq_shadow:latest
    if ! (CCR_MODE="-checker" . ./scripts/ccr.sh && docker compose -f ./compose.yml create qonq_shadow); then
        echo "|> Error: could not create the built [qonq_shadow] container using the ccr.sh script to use Podman Service as the compose tool"
        echo && echo
        return 1
    fi
    echo "|> Created the built [qonq_shadow] container using the ccr.sh script to use Podman Service as the compose tool with success. Proceeding..."
    echo && echo

    # check created containers
    CCR_MODE="-checker" . ./scripts/ccr.sh && docker compose ps --all

    # check for the qonq_shadow image at localhost:5000/qonq_shadow
    podman images | grep "localhost:5000/qonq_shadow" | awk '{print $1}'

    # copy qonq_shadow tarball into the ./artifacts/packaging directory.
    mkdir -p "${PACKAGING_ART_DIR:-[EMPTY_VARIABLE]}"
    if ! podman cp qonq_shadow:/app/shadow-tarball-pkg.tar.gz "${PACKAGING_ART_DIR:-[EMPTY_VARIABLE]}"; then
        echo "|> Error: could not copy the full shadow tarball to the PACKAGING_ART_DIR=${PACKAGING_ART_DIR:-[EMPTY_VARIABLE]} filepath. Exiting now..."
        return 1
    fi
    echo "|> Copied the [qonq_shadow] full shadow tarball to the PACKAGING_ART_DIR=${PACKAGING_ART_DIR:-[EMPTY_VARIABLE]} filepath with success. Proceeding... "
