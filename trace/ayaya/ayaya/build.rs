...
/// This crate has a runtime dependency on artifacts produced by the `ayaya-ebpf` crate.
...
fn main() {
    ...
    let ebpf_package = packages
        .into_iter()
        .find(|Package { name, .. }| name == "ayaya-ebpf")
        .unwrap();
    ...
    if build_ebpf {
        ...
        // We have a build-dependency on `ayaya-ebpf`, so cargo will automatically rebuild us
        // if `ayaya-ebpf`'s *library* target or any of its dependencies change. Since we
        // depend on `ayaya-ebpf`'s *binary* targets, that only gets us half of the way. This
        ...
        let ebpf_target_dir = out_dir.join("ayaya-ebpf");
